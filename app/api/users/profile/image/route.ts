...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!allowedTypes.includes(file.type)) {
      ...
      return NextResponse.json(
        { error: 'Format file tidak didukung. Silakan unggah file gambar (JPEG, PNG, WebP).' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Gagal membuat direktori upload.', details: error instanceof Error ? error.message : 'Unknown error' },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Gagal menyimpan file yang diunggah.', details: error instanceof Error ? error.message : 'Unknown error' },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      try {
        ...
        console.log(`API: Removed uploaded file ${absolutePath} after database error`);
      } catch (unlinkError) {
        console.error('API: Failed to remove uploaded file after database error:', unlinkError);
      }
      
      return NextResponse.json(
        { error: 'Gagal memperbarui profil dalam database.', details: error instanceof Error ? error.message : 'Unknown error' },
        { status: 500 }
      );
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { error: 'Gagal mengunggah foto profil.', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
} 