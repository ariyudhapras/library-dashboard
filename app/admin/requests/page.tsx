...
export default function AdminRequestsPage() {
  ...
  const handleStatusChange = async (loanId: number, status: string) => {
    try {
      ...
      toast.success(`Status peminjaman berhasil diubah menjadi ${status === 'APPROVED' ? 'Disetujui' : 'Ditolak'}`)
      
      // Refresh the page data
      router.refresh()
    } catch (error) {
      console.error('Error updating loan status:', error)
      toast.error(error instanceof Error ? error.message : 'Gagal mengubah status peminjaman')
    } finally {
      setIsProcessing(false)
    }
  }

  const openConfirmDialog = (loan: BookLoan, action: string, status: string) => {
                    ...
                    <TableBody>
                      {filteredLoans.map((loan) => (
                        <TableRow key={loan.id}>
                          ...
                          <TableCell>
                            {loan.status === 'PENDING' && (
                              <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">Menunggu</Badge>
                            )}
                            {loan.status === 'APPROVED' && (
                              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Disetujui</Badge>
                            )}
                            {loan.status === 'REJECTED' && (
                              <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Ditolak</Badge>
                            )}
                            {loan.status === 'RETURNED' && (
                              <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Dikembalikan</Badge>
                            )}
                            {loan.status === 'LATE' && (
                              <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">Terlambat</Badge>
                            )}
                          </TableCell>
                          <TableCell className="text-right">
                            {loan.status === 'PENDING' && (
                              <div className="flex justify-end gap-2">
                                <Button 
                                  ...
                                  onClick={() => openConfirmDialog(loan, "approve", "APPROVED")}
                                >
                                ...
                                <Button 
                                  ...
                                  onClick={() => openConfirmDialog(loan, "reject", "REJECTED")}
                                >
                            ...
                            {loan.status === 'APPROVED' && (
                              <Button 
                                ...
                                variant="outline" 
                                className="h-8"
                                onClick={() => openConfirmDialog(loan, "return", "RETURNED")}
                              >
                                Tandai Kembali
                              </Button>
                            )}
      ...
      <Dialog open={confirmDialog.open} onOpenChange={(open) => setConfirmDialog({ ...confirmDialog, open })}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {confirmDialog.action === "approve" && "Apakah Anda yakin ingin menyetujui peminjaman buku ini?"}
              {confirmDialog.action === "reject" && "Apakah Anda yakin ingin menolak peminjaman buku ini?"}
              {confirmDialog.action === "return" && "Apakah Anda yakin buku ini telah dikembalikan?"}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">Peminjam: {selectedLoan.user.name}</div>
                  <div className="text-sm text-muted-foreground">
                    Periode: {format(new Date(selectedLoan.borrowDate), "d MMM yyyy", { locale: id })} - {format(new Date(selectedLoan.returnDate), "d MMM yyyy", { locale: id })}
                  </div>
                </div>
              </div>
              
              {confirmDialog.action === "approve" && selectedLoan.book.stock <= 0 && (
                <div className="flex items-center gap-2 p-3 my-2 bg-amber-50 text-amber-700 rounded-md border border-amber-200">
                  <AlertCircle className="h-4 w-4" />
                  <span>Stok buku ini habis. Menyetujui peminjaman ini bisa menyebabkan stok negatif.</span>
                </div>
              )}
            </div>
          )}
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => setConfirmDialog({ open: false, action: "", status: "" })}
            >
              Batal
            </Button>
            <Button 
              variant={confirmDialog.action === "reject" ? "destructive" : "default"}
              onClick={() => selectedLoan && handleStatusChange(selectedLoan.id, confirmDialog.status)}
