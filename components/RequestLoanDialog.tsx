...
export default function RequestLoanDialog({ open, onOpenChange, book, onSuccess }: RequestLoanDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] w-screen h-screen max-w-none rounded-none p-0 gap-0 shadow-lg overflow-y-auto md:max-h-[90vh]">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle className="text-xl text-center">Ajukan Peminjaman Buku</DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground text-center mt-1 mb-2">
            Anda akan mengajukan peminjaman buku berikut.<br />Peminjaman akan diproses oleh petugas perpustakaan.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="px-6 py-6 grid gap-6">
            ...
            <div className="flex gap-6 pb-6 border-b">
              ...
              <div className="flex flex-col justify-between flex-1">
                <div>
                  <h3 className="text-lg font-semibold tracking-tight mb-1">{book.title}</h3>
                  <p className="text-sm text-muted-foreground">Karya: {book.author}</p>
                  {book.year && <p className="text-sm text-muted-foreground">Tahun: {book.year}</p>}
                  {book.description && <p className="mt-3 text-sm text-slate-600 line-clamp-3">{book.description}</p>}
                </div>
